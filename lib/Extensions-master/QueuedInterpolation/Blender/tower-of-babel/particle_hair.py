...
class Hair():
    def __init__(self, object):
        ...
        self.unit = object.data.unit
        self.strandWidth = object.data.strandWidth
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    ...
    def to_script_file(self, file_handler, typescript_file_handler, kids, indent, exporter):
        ...
        file_handler.write(indent2 + var + '.unit = "' + self.unit + '";\n')
        file_handler.write(indent2 + var + '.strandWidth = "' + self.strandWidth + '";\n')
...
#        file_handler.write(indent2 + var + '.cornerOffset = ' + format_f(self.cornerOffset) + ';\n')
        ...
        if isRootMesh:
            ...
            file_handler.write(indent + '}\n')
#===============================================================================
bpy.types.Mesh.unit = bpy.props.EnumProperty(
    name='Unit',
    description='Need to know the unit this is based on to make strand width',
    items = ((DECIMETER , 'Decimeter' , 'Use Decimeters for strand width'),
             (METER     , 'Meter'     , 'Use Meter for strand width'),
             (INCH      , 'Inch'      , 'Use Inch for strand width'),
             (CENTIMETER, 'Centimeter', 'Use Centimeter for strand width')
            ),
    default = DECIMETER
)

bpy.types.Mesh.strandWidth = bpy.props.EnumProperty(
    name='Strand Width',
    description='',
    items = ((THIN_WIDTH  , 'Thin'  , 'Thin strands'),
             (MEDIUM_WIDTH, 'Medium', 'Medium strands'),
             (THICK_WIDTH , 'Thick' , 'Thick strands')
            ),
