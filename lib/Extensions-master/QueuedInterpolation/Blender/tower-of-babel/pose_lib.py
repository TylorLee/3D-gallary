...
class PoseLibExporter:
    def execute(self, context, filepath, bpySkeleton):
        ...
        write_js_module_header(file_handler, nameSpace, False, False)
...
# broken out so libraries can be in-lined to a scene export
    def perform(self, scene, file_handler, bpySkeleton):
        ...
        file_handler.write(indent + 'var lib = QI.SkeletonPoseLibrary.createLibrary("' + libraryName + '",  new _V('+ format_vector(dimensions) + '), "'  + self.getRootBone() + '", boneLengths);\n')
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    def hasMultipleRootBones(self):
        ...
        return count > 1
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# have problem returning arrays of bones, just need name
